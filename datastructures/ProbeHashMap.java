...
public class ProbeHashMap<K, V> extends AbstractHashMap<K, V> {
	...
	protected void createTable() {
		table = new MapEntry[capacity]; // safe cast
	}
	...
	/**
	 * Searches for an entry with key equal to k (which is known to have hash
	 * value h), returning the index at which it was found, or returning -(a+1)
	 * where a is the index of the first empty or available slot that can be
	 * used to store a new such entry.
	 *
	 * @param h
	 *            the precalculated hash value of the given key
	 * @param k
	 *            the key
	 * @return index of found entry or if not found, value -(a+1) where a is
