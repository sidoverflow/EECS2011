...
public class AVLTreeMap<K, V> extends TreeMap<K, V> {
	...
	/**
	 * Recomputes the height of the given position based on its children's
	 * heights.
	 */
		...
		for (Position<Entry<K, V>> p : tree.positions()) {
			if (isInternal(p)) {
				if (p.getElement() == null)
					System.out
							.println("VIOLATION: Internal node has null entry");
				else if (height(p) != 1 + Math.max(height(left(p)),
						height(right(p)))) {
					System.out
							.println("VIOLATION: AVL unbalanced node with key "
									+ p.getElement().getKey());
					dump();
					return false;
				}
