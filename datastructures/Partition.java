...
public class Partition<E> {
	...
	private Locator<E> validate(Position<E> pos) {
		...
		if (!loc.validate(this))
			throw new IllegalArgumentException(
					"Position does not belong to this structure");
		return loc;
	}
	...
	/**
	 * Finds the cluster containing the element identified by Position p and
	 * returns the Position of the cluster's leader.
	 */
	public Position<E> find(Position<E> p) {
		Locator<E> loc = validate(p);
		if (loc.parent != loc)
			loc.parent = (Locator<E>) find(loc.parent); // overwrite parent
														// after recursion
		return loc.parent;
	}

	/**
	 * Merges the clusters containing elements with positions p and q (if
	 * distinct).
	 */
