...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
	...
	/**
	 * A specialized version of the LinkedBinaryTree class with additional
	 * mutators to support binary search tree operations, and a specialized node
	 * class that includes an auxiliary instance variable for balancing data.
	 */
	protected static class BalanceableBinaryTree<K, V> extends
		...
		// Override node factory function to produce a BSTNode (rather than a
		// Node)
		@Override
		...
		public void rotate(Position<Entry<K, V>> p) {
			...
			if (x == y.getLeft()) {
				relink(y, x.getRight(), true); // x's right child becomes y's
												// left
				relink(x, y, false); // y becomes x's right child
			} else {
				relink(y, x.getLeft(), false); // x's left child becomes y's
												// right
		...
		/**
		 *
		 * Returns the Position that becomes the root of the restructured
		 * subtree.
		 *
		 ...
		 * </pre>
		 * 
		 * The subtree will be restructured so that the node with key b becomes
		 * its root.
		 *
		 * <pre>
		 *           b
	 ...
	 *            a target key
	 * @param p
	 *            a position of the tree serving as root of a subtree
	...
	public V remove(K key) throws IllegalArgumentException {
		...
		if (isExternal(p)) { // key not found
			...
			V old = p.getElement().getValue();
			if (isInternal(left(p)) && isInternal(right(p))) { // both children
																// are internal
				Position<Entry<K, V>> replacement = treeMax(left(p));
				set(p, replacement.getElement());
				p = replacement;
			} // now p has at most one child that is an internal node
			Position<Entry<K, V>> leaf = (isExternal(left(p)) ? left(p)
					: right(p));
	...
	public Entry<K, V> lowerEntry(K key) throws IllegalArgumentException {
		...
		if (isInternal(p) && isInternal(left(p)))
			return treeMax(left(p)).getElement(); // this is the predecessor to
													// p
	...
	private void subMapRecurse(K fromKey, K toKey, Position<Entry<K, V>> p,
		...
		if (isInternal(p))
			if (compare(p.getElement(), fromKey) < 0)
				// p's key is less than fromKey, so any relevant entries are to
				// the right
				subMapRecurse(fromKey, toKey, right(p), buffer);
			else {
				subMapRecurse(fromKey, toKey, left(p), buffer); // first
																// consider left
																// subtree
				if (compare(p.getElement(), toKey) < 0) { // p is within range
					buffer.add(p.getElement()); // so add it to buffer, and
												// consider
					subMapRecurse(fromKey, toKey, right(p), buffer); // right
																		// subtree
																		// as
																		// well
				}
	...
	private void dumpRecurse(Position<Entry<K, V>> p, int depth) {
		String indent = (depth == 0 ? "" : String.format("%" + (2 * depth)
				+ "s", ""));
		if (isExternal(p))
