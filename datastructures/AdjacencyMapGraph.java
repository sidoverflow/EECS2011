...
public class AdjacencyMapGraph<V, E> implements Graph<V, E> {
	...
	/**
	 * Returns the number of edges for which vertex v is the origin. Note that
	 * for an undirected graph, this is the same result returned by inDegree(v).
	 * 
	...
	public Iterable<Edge<E>> outgoingEdges(Vertex<V> v)
			throws IllegalArgumentException {
		InnerVertex<V> vert = validate(v);
		return vert.getOutgoing().values(); // edges are the values in the
											// adjacency map
	}

	/**
	 * Returns the number of edges for which vertex v is the destination. Note
	...
	public Iterable<Edge<E>> incomingEdges(Vertex<V> v)
			throws IllegalArgumentException {
		InnerVertex<V> vert = validate(v);
		return vert.getIncoming().values(); // edges are the values in the
											// adjacency map
	}
	...
	public Edge<E> getEdge(Vertex<V> u, Vertex<V> v)
		...
		return origin.getOutgoing().get(v); // will be null if no edge from u to
											// v
	}
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if u or v are invalid vertices, or if an edge already exists
	 *             between u and v.
	 */
	@Override
	...
	public String toString() {
		...
		for (Vertex<V> v : vertices) {
			...
			for (Edge<E> e : outgoingEdges(v))
				sb.append(String.format(" (%s, %s)", opposite(v, e)
