...
public class RBTreeMap<K, V> extends TreeMap<K, V> {
	...
	private void resolveRed(Position<Entry<K, V>> p) {
		Position<Entry<K, V>> parent, uncle, middle, grand; // used in case
															// analysis
		parent = parent(p);
	...
	/**
	 * Remedies a presumed double-black violation at the given (nonroot)
	 * position.
	 */
	private void remedyDoubleBlack(Position<Entry<K, V>> p) {
		...
		if (isBlack(y)) {
			if (isRed(left(y)) || isRed(right(y))) { // Case 1: trinode
														// restructuring
				Position<Entry<K, V>> x = (isRed(left(y)) ? left(y) : right(y));
				Position<Entry<K, V>> middle = restructure(x);
				setColor(middle, isRed(z)); // root of restructured subtree gets
											// z's old color
	...
	private int sanityRecurse(Position<Entry<K, V>> p) {
		if (isExternal(p)) {
			if (isRed(p))
				return -1; // invalid; should be black
			else
				return 0; // valid, with black-depth 0
		} else {
			if (isRoot(p) && isRed(p))
				return -1; // root must be black
			Position<Entry<K, V>> left = left(p);
			Position<Entry<K, V>> right = right(p);
			if (isRed(p) && (isRed(left) || isRed(right)))
				return -1; // cannot have double red
			...
			if (a != b)
				return -1; // two subtrees must have identical black depth

			return a + (isRed(p) ? 0 : 1); // our black depth might be one
											// greater
		}
	}
}
