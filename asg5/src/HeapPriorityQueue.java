...
public class HeapPriorityQueue<K, V> extends AbstractPriorityQueue<K, V> {
	...
	protected void upheapRec(int j) {
		...
		if (j > 1 && compare(heap.get(j), heap.get(parent(j))) < 1) {
			swap(j, parent(j));
			upheapRec(parent(j));
		}
	...
	protected void downheapRec(int j) {
		...
		if (hasLeft(j)) {
			int minChild = left(j);
			if (hasRight(j)) {
				if (compare(heap.get(left(j)), heap.get(right(j))) >= 1) {
					minChild = right(j);
				}
			}
			if (compare(heap.get(minChild), heap.get(j)) < 0) {
				swap(j, minChild);
				downheapRec(minChild);
			}
		}
	...
	public int updateKey(K currentKey, K newKey) {
		...
		for (int i = 1; i < heap.size(); i++) {
			if (heap.get(i).getKey().equals(currentKey)) {
				PQEntry<K, V> tmp = new PQEntry(heap.get(i).getKey(), heap.get(i).getValue());
				tmp.setKey(newKey);
				heap.set(i, tmp);
				upheapRec(i);
				downheapRec(i);
